...
//	Console::WriteLine(">>>>>> Total time taken for order processing {0} ms ",((maxProcessTime-minProcessTime) / 10000));
	...
	unsigned __int64 processReadTime = 0;
	unsigned __int64 processWriteTime = 0;
	unsigned __int64 processChangesetTime = 0;
	...
	for (int i = 0; i < fillProcessorWorkers.size(); i++)
	{
		...
		processReadTime += fillProcessorWorkers[i]->processReadTime;
		processWriteTime += fillProcessorWorkers[i]->processWriteTime;
		processChangesetTime += fillProcessorWorkers[i]->processChangesetTime;
